/**
 * self-memorycontrol — plug-and-play context management + structured memory
 * for DeepSeek Harness.
 *
 * Host half: registers the memory_* tools (skipping any the harness already
 * provides), the memory guidance prompt section, and (in web profiles) the
 * browser UI JSON API. Durable storage is the same local JSON document the
 * harness's built-in memory uses ($DSH_HOME/memory), so data is shared.
 *
 * Install:  dsh plugin --profile web add self-memorycontrol
 *           (or a git/path spec, e.g. github:whynotcooper/memorycontrol)
...
export const name = "self-memorycontrol";
...
export function apply(ctx, config) {
  ...
  ctx.__selfMemory = { store, api, context: cm };
  ...
  ctx.effect(() => () => {}, "self-memorycontrol: store");

  if (cfg.registerTools !== false) {
    registerTools(ctx, api);
  }
  if (cfg.prompt !== false) {
    ctx.systemPrompt.section({ name: "self-memory:control", order: 116, text: PROMPT });
  }
  if (cfg.webApi !== false) {
