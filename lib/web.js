 ...
 *
 * POST /plugins/self-memorycontrol/api
...
/**
 * Register the self-memorycontrol API route.
...
export function registerWebApi(ctx, api, { resolveWorkspace = () => null } = {}) {
  ...
  const disposer = webServer.register({ kind: "prefix", path: "/plugins/self-memorycontrol/api", handler });
  ctx.effect(() => disposer);
  return true;
}
