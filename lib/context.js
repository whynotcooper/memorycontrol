/**
 * ContextManager — the context-management engine of memorycontrol.
 *
...
export class ContextManager {
  ...
  warn(kind, error) {
    console.warn(`[memorycontrol] context ${kind}: ${error?.message ?? error}`);
  }
  ...
  async extractFromText(session, text, reason) {
    ...
    const messages = [createUserMessage({
      ...
      source: { kind: "plugin", plugin: "memorycontrol" },
    })];
    const options = {
  ...
  async archiveSession(exec, { summarize = false, maxChars = 60000 } = {}) {
    ...
    if (summarize && this.ctx.llm) {
      ...
      if (target) {
        ...
        const messages = [createUserMessage({ content: [{ type: "text", text: instruction }], source: { kind: "plugin", plugin: "memorycontrol" } })];
        const assembler = new BlockAssembler();
