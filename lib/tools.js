/**
 * self-memorycontrol tool definitions (memory_*), registered through
 ...
 * A same-named tool already visible in this scope (e.g. the harness's own
 * built-in memory_* tools in the web composition) is NOT re-registered —
 * they share the same names, query DSL, and storage, so re-registering would
 * only create duplicates or a registration error. The context engine and web
 * UI are unaffected.
...
export function registerTools(ctx, api) {
  ...
  const register = (def) => {
    try {
      ...
      // tools.get unavailable or throws — register anyway
    }
    T(def);
  };
  ...
  register({
    name: "memory_import",
    description: "Import memory entries from a self-memorycontrol export file (or a bare array of entries). Existing ids are skipped unless replace=true.",
    parameters: {
  ...
  if (skipped.length > 0) {
    console.warn(`[self-memorycontrol] memory_* tools already provided by this deployment, skipped re-registering: ${skipped.join(", ")}`);
  }

  // --- context management tools (self-memorycontrol v0.2) ---

  const CONTEXT_STATUS_OUTPUT = {
    schema: {
      type: "object",
      additionalProperties: true,
      properties: {
    ...
    render: (_args, value) => {
      ...
      const lines = [
        `conversation: ${conv.surfaceChars ?? 0} chars ≈ ${conv.estimatedTokens ?? 0} tokens · ${conv.compactions ?? 0} compactions`,
        `memory: ${mem.total ?? 0} entries (${mem.expired ?? 0} expired) · archives ${ctx.archives ?? 0} · extractions ${ctx.extractions ?? 0}`,
        `context module: ${ctx.enabled ? "on" : "off"} (extract ${ctx.extractEnabled ? "on" : "off"}, archive ${ctx.archiveEnabled ? "on" : "off"}, recall ${ctx.recallEnabled ? "on" : "off"}, recall budget ${ctx.recallBudgetChars ?? 0} chars)`,
        `root: ${ctx.root ?? "?"}`,
      ];
      if ((conv.surfaceChars ?? 0) > 12000) {
        lines.push("hint: the conversation is large — use context_archive to snapshot it, then /compact.");
      }
      return [{ type: "text", text: lines.join("\n") }];
    },
  };

  const CONTEXT_ARCHIVE_OUTPUT = {
    schema: {
      type: "object",
      additionalProperties: true,
      properties: {
    ...
    render: (_args, value) => [{
      type: "text",
      text: `archived conversation to [${value.archiveId}] (${value.chars} chars${value.summarized ? ", summarized" : ""}); extracted ${value.extracted} durable entr${value.extracted === 1 ? "y" : "ies"}. You can now /compact or continue lean.`,
    }],
  };

  register({
    name: "context_status",
    description: "Report conversation size (chars, estimated tokens, compaction count) plus memory and context-module telemetry (archives, extractions, recall budget). Use this to decide when to archive or /compact.",
    parameters: {},
    output: CONTEXT_STATUS_OUTPUT,
    execute(_args, exec) {
      if (!api.context) throw new Error("context module is disabled (context.enabled=false)");
      return api.context.status(exec);
    },
    presentCall: () => present("Context status", "read"),
  });

  register({
    name: "context_archive",
    description: "Snapshot the current conversation into a searchable memory archive and extract durable knowledge from it. Call this before a long session ends or before /compact so nothing is lost. Returns the archive entry id (searchable with memory_search kind:context) and how many durable entries were extracted.",
    parameters: {
      summarize: { type: "boolean", description: "Ask the model to write a summary into the archive (default false)." },
      max_chars: { type: "number", description: "Max conversation characters to archive (default 60000)." },
    },
    output: CONTEXT_ARCHIVE_OUTPUT,
    execute(args, exec) {
      if (!api.context) throw new Error("context module is disabled (context.enabled=false)");
      return api.context.archiveSession(exec, { summarize: Boolean(args.summarize), maxChars: args.max_chars });
    },
    presentCall: (args) => present("Archive context", "other", args?.summarize ? "summarize" : ""),
  });
}
